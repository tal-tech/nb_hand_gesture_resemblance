...
if __name__ == '__main__':
    ...
    folder_path = 'util/'
    det=Det_dada_hand()
    filename1 = "gesture1.mp4"
    video_path = os.path.join(folder_path, filename1)
            
    filename2 = "gesture2.mp4"
    video_path2 = os.path.join(folder_path, filename2)
    # 打开视频文件
    ...
    # 对两个列表进行迭代，只迭代最小长度次数
    for dict1, dict2 in zip(list1[:min_length], list2[:min_length]):
        if dict1['hand_gesture'] == dict2['hand_gesture']:
            same_gesture_count += 1

    # 计算相同手势占比并返回结果
    similarity_ratio = same_gesture_count / min_length
    
    print("两个手势视频的相似度"+str(similarity_ratio))
